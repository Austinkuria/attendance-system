...
const AttendanceManagement = () => {
  ...
  const disabledTextColor = "#8c8c8c"; // Gray for disabled button text visibility

  // LocalStorage sync for currentSession
  useEffect(() => {
    if (currentSession) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setUnits(unitsData || []);
        if (unitsData?.length && !selectedUnit && !currentSession) {
  ...
  // Restored original createSession logic
